...
impl CoreState {
    /// 打开交互式远程 Shell 的稳定核心入口。
    #[cfg_attr(not(feature = "desktop"), allow(dead_code))]
    pub(crate) fn open_shell_action(&mut self, host_id: HostId) -> AppUpdateOutcome {
        self.open_shell(host_id)
    }

    /// 从最近连接记录重新打开远程 Shell 的稳定核心入口。
    #[cfg_attr(not(feature = "desktop"), allow(dead_code))]
    pub(crate) fn open_recent_connection_action(&mut self, host_id: HostId) -> AppUpdateOutcome {
        self.open_recent_connection(host_id)
    }

    /// 重新连接已有远程 Shell 标签页的稳定核心入口。
    #[cfg_attr(not(feature = "desktop"), allow(dead_code))]
    pub(crate) fn reconnect_shell_action(&mut self, session_id: SessionId) -> AppUpdateOutcome {
        self.reconnect_shell(session_id)
    }
