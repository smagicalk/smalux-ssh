...
use crate::model::{SessionId, SessionTab, SftpBrowserState, TransferTask, TunnelRuntimeState};
...
impl SessionManager {
    ...
    pub(crate) fn upsert_sftp_browser(&mut self, browser: SftpBrowserState) {
        if let Some(existing) = self
            .sftp_browsers
            .iter_mut()
