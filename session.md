...
- 当前优先级：在不破坏现有行为的前提下继续收敛架构；UI 可以继续打磨，但不要把核心逻辑写进 Slint。
...
- 最近验证通过：
  ...
  - `cargo test --color never app::view_model::tests`，`30 passed`
  - `cargo test --color never`，`436 passed`
  - `git diff --check` 无空白错误，只有 Windows LF/CRLF 提示
- 2026-06-09 片段页最近验证：
  - `cargo check --color never` 通过。
  - `cargo build --color never` 产物已更新并启动预览。
  - `cargo test --lib --color never parse_target_keys_accepts_multi_select_payload` 通过。
  - `cargo test --lib --color never snippet_target_messages_create_share_update_split_and_remove_targets` 通过。
  - `cargo test --lib --color never snippet_group_delete_rejects_non_empty_group_and_move_snippet_works` 在编译阶段超时，未出现断言失败；残留 cargo/rustc 已清理。
- 最近预览：
  - 已先关闭旧进程，再重新 `cargo build --color never`
  - 新版 `target/debug/smagicalssh.exe` 已启动，最近预览进程为 `11720`
- 编码抽样正常：
  - `session.md` 首字节为 `35,32,115`，无 BOM
  - 新增 Slint wrapper 文件首字节为 `105,109,112`，无 BOM
  - 中文注释抽样正常
...
- 当前工作区大量未跟踪文件是前面拆分和功能开发结果，不要删除。
- 如果要提交，先让用户确认 commit message；用户之前提醒过 commit 要求，但本轮没有要求 commit。
- 最近预览：直接启动 `target/debug/smagicalssh.exe` 成功，进程 `24200` 保持运行，窗口应已打开。
