...
- 本轮新增：继续收口 SSH 客户端核心，`smagical-ssh-client-core` 继续承载 RemoteTunnel 运行句柄、SOCKS5 握手解析、SFTP 纯映射 helper、channel 消息映射、handler、ssh-agent 身份选择、主机密钥校验策略和 `russh` 客户端配置。
...
- 本轮待提交：提交 RemoteTunnel 运行句柄核心拆分并整理恢复记录
