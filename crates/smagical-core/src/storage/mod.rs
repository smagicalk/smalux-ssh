...
pub trait CredentialRepository: Send + Sync {
    /// 获取全部凭据记录
    async fn list_all(&self) -> StorageResult<Vec<crate::domain::credential::CredentialRecord>>;

    /// 按分类查询 (Key / Password / Agent / Certificate)
    async fn list_by_type(&self, cred_type: crate::domain::credential::CredentialType) -> StorageResult<Vec<crate::domain::credential::CredentialRecord>>;

    /// 根据唯一 ID 查询单条凭据记录
    async fn get_by_id(&self, id: &str) -> StorageResult<Option<crate::domain::credential::CredentialRecord>>;

    /// 模糊搜索凭据 (按名称、用户名、公钥指纹、备注)
    async fn search(&self, query: &str) -> StorageResult<Vec<crate::domain::credential::CredentialRecord>>;

    /// 获取引用/绑定了该凭据的主机 ID 列表
    async fn get_bound_hosts(&self, id: &str) -> StorageResult<Vec<String>>;

    /// 保存或更新凭据记录
    async fn save(&self, record: &crate::domain::credential::CredentialRecord) -> StorageResult<()>;

    /// 批量保存凭据记录
    async fn save_batch(&self, records: &[crate::domain::credential::CredentialRecord]) -> StorageResult<()>;
...
pub trait SnippetRepository: Send + Sync {
    /// 获取全部代码片段记录
    async fn list_all(&self) -> StorageResult<Vec<crate::domain::snippet::SnippetRecord>>;

    /// 按所属文件夹分组获取代码片段 (None 表示根目录下)
    async fn list_by_group(&self, group_id: Option<&str>) -> StorageResult<Vec<crate::domain::snippet::SnippetRecord>>;

    /// 根据唯一 ID 查询代码片段
    async fn get_by_id(&self, id: &str) -> StorageResult<Option<crate::domain::snippet::SnippetRecord>>;

    /// 模糊搜索代码片段 (按标题、命令、标签、备注)
    async fn search(&self, query: &str) -> StorageResult<Vec<crate::domain::snippet::SnippetRecord>>;

    /// 保存或更新代码片段
    async fn save(&self, record: &crate::domain::snippet::SnippetRecord) -> StorageResult<()>;

    /// 批量保存代码片段
    async fn save_batch(&self, records: &[crate::domain::snippet::SnippetRecord]) -> StorageResult<()>;
    ...
    /// 获取全部代码片段层级分组
    async fn list_groups(&self) -> StorageResult<Vec<crate::domain::snippet::SnippetGroupRecord>>;

    /// 根据唯一 ID 查询分组
    async fn get_group_by_id(&self, id: &str) -> StorageResult<Option<crate::domain::snippet::SnippetGroupRecord>>;

    /// 保存或更新分组
    async fn save_group(&self, group: &crate::domain::snippet::SnippetGroupRecord) -> StorageResult<()>;
...
pub trait TunnelRepository: Send + Sync {
    /// 获取全部隧道与代理记录
    async fn list_all(&self) -> StorageResult<Vec<crate::domain::tunnel::TunnelRecord>>;

    /// 按类型获取 (Local / Remote / Dynamic / JumpHost / ProxyServer)
    async fn list_by_type(&self, tunnel_type: crate::domain::tunnel::TunnelType) -> StorageResult<Vec<crate::domain::tunnel::TunnelRecord>>;

    /// 根据唯一 ID 查询单条记录
    async fn get_by_id(&self, id: &str) -> StorageResult<Option<crate::domain::tunnel::TunnelRecord>>;

    /// 模糊搜索隧道 (按名称、端口、目标主机、备注)
    async fn search(&self, query: &str) -> StorageResult<Vec<crate::domain::tunnel::TunnelRecord>>;

    /// 保存或更新隧道记录
    async fn save(&self, record: &crate::domain::tunnel::TunnelRecord) -> StorageResult<()>;

    /// 批量保存隧道记录
    async fn save_batch(&self, records: &[crate::domain::tunnel::TunnelRecord]) -> StorageResult<()>;
