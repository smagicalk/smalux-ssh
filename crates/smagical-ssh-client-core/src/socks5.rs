...
/// 读取 SOCKS5 CONNECT 握手并返回目标地址。
pub async fn read_socks5_target<S>(stream: &mut S) -> Result<Socks5Target, String>
where
...
/// 写入 SOCKS5 CONNECT 成功响应。
pub async fn write_socks5_success<S>(stream: &mut S) -> std::io::Result<()>
where
    S: AsyncWrite + Unpin,
{
    stream
